...
1 => index; // user provides section number (same as index value)
...
while( second_i <= end )
{
    ...
    if( times[index] == second_i ) // only gets triggered at each timing interval
    {
        ...
        if( clar_spkr_freqs[index] > 350)
        {
            clar_amps[index] * 0.7 => targetGain;
            <<< clar_amps[index] * 0.7 >>>;
        }
        else if( clar_spkr_freqs[index] > 250 )
        {
            clar_amps[index] * 0.8 => targetGain;
            <<< clar_amps[index] * 0.8 >>>;
        }
        else if( clar_spkr_freqs[index] > 150 )
            clar_amps[index] * 0.9 => targetGain;
        else clar_amps[index] * 1.05 => targetGain;
        targetGain => s.gain;
